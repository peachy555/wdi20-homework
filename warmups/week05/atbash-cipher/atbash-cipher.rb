...
cipher = AtbashOriginal.new(word)
puts cipher.encode



##### Here is the version we came up with together during the warmup
#
# We've changed the code so that you can encode a different string
# each time you call .encode, instead of only one string per instance
#
# We also turned our alphabet arrays into @@class variables, shared
# by all instances, so we don't need an initialize method at all.
